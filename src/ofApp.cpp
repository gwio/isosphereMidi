...
void ofApp::setup(){
    ...
    trigger = synth.addParameter("trigger");
    midiNote = synth.addParameter("midiNumber");
    note = ControlMidiToFreq().input(midiNote);
    synthA = SineWave().freq(note);
    adsr1 = ADSR(0.005,0.02, 0, 0.0).trigger(trigger).doesSustain(true).legato(true);
    
    synth.setOutputGen( (synthA*adsr1) );
    sphere.link_trigger(&synth);
...
void ofApp::draw(){
    ...
    ofDrawBitmapString(ofToString(sphere.ico_sphere.getNumVertices()), 100,100);
    ofPopStyle();
